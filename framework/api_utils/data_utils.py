...
class DataUtils:
    ...
    def get_max_element_in_list_of_dict(iterable: Sequence[dict], dict_field: str) -> int:
        ...
        framework.utils.logger.info(f'Getting max value from all dicts in list "{iterable}" by '
                    f'field "{dict_field}"')
    ...
    def get_min_element_in_list_of_dict(iterable: Sequence[dict], dict_field: str) -> int:
        ...
        framework.utils.logger.info(f'Getting min value from all dicts in list "{iterable}" by '
                    f'field "{dict_field}"')
    ...
    def elements_in_json_list_sorted_by_field(
        ...
        """
        Returns True if collection 'json_data' sorted by value in field with description that sends to 'dict_field' param.
        If param 'descending' is True, method will check descending of collection. elements_count_to_check = 0 - it
        means check all items. If this param greater than 0, method will check only first 'elements_count_to_check'
        elements.
        """
        ...
        compare = int.__lt__ if descending else int.__gt__
